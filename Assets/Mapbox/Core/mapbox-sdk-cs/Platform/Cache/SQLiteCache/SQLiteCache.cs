namespace Mapbox.Platform.Cache
{
    ...
    public class SQLiteCache : ICache, IDisposable
    {
        ...
        public void Add(CanonicalTileId tileId, CacheItem item, bool forceInsert = false)
        {
            ...
            try
            {
                ...
                if (1 != rowsAffected)
                {
                    throw new Exception(string.Format("tile [{0} / {1}] was not inserted, rows affected:{2}", tileId, rowsAffected));
                }
            }
            catch (Exception ex)
            {
                Debug.LogErrorFormat("Error inserting {0} {1} {2} ", tileId, ex);
            }
        ...
        public CacheItem Get(CanonicalTileId tileId)
        {
            ...
            {
                Debug.LogErrorFormat("error getting tile {1} {2} from cache{0}{3}", Environment.NewLine, tileId, ex);
                return null;
            }
            if (null == tile)
            {
